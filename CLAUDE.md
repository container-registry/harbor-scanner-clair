...
- **Register with `use_internal_addr: false` unless the adapter shares Harbor's namespace.**
  With it true Harbor puts its internal core hostname in `registry.url`
  (`http://harbor-harbor-next-core`), which resolves nowhere else, and every scan fails fast at
  the manifest fetch with `[network] ... no such host`. With the external URL Clair pulls
  through the public ingress and follows the 307 to object storage, verified against R2. The
  registry URL and token are per request, so one deployment serves several Harbors.
- **An empty report is usually Clair's data.** The Debian updater carries only the releases in
  the live security tracker, so jessie through bullseye (the last since 2026-09-01) match
  nothing; images with no detected distribution (busybox, scratch-like, prometheus) yield no
  packages at all; UBI and Oracle need `rhel` and `oracle` in `updaters.sets`. Read Clair's
  configuration before reading the transformer.
