...
|------|---------|-------------|
| `SCANNER_LOG_LEVEL` | `info` | `trace`, `debug`, `info`, `warn`, `error`, and, carried over from 1.x, `warning`, `fatal` and `panic`. `trace` means debug, `fatal` and `panic` mean error, and an unrecognized value means info. |
...
| `SCANNER_STORE_SCAN_JOB_TTL` | derived | How long a scan job and its report live. Unset, it is derived as `2 x (SCANNER_CLAIR_INDEX_TIMEOUT + SCANNER_CLAIR_REPORT_RETRY_TIMEOUT + 60s) + 3s`, which is `32m3s` at the default Clair timeouts and grows with them. See the invariant below. |
...
`SCANNER_STORE_REDIS_URL` and `SCANNER_STORE_REDIS_SCAN_JOB_TTL` are rejected at startup rather
than ignored, as is `SCANNER_CLAIR_DATABASE_URL`: an upgraded deployment that still set them would
otherwise connect nowhere and fail on the first scan. The rest of the 1.x pool settings
(`SCANNER_STORE_REDIS_POOL_*`, `SCANNER_STORE_REDIS_NAMESPACE`) are not read at all, so drop them
in the same change.
...
Leave **Use internal registry addr** unchecked unless the adapter runs inside Harbor's own
namespace. That box decides what Harbor puts in `registry.url`. Checked (`use_internal_addr:
true`), Harbor sends its internal core hostname, `http://harbor-harbor-next-core` for instance,
which resolves nowhere else, and every scan fails within a second:

```
[network] fetching the artifact manifest: ... lookup harbor-harbor-next-core: no such host
```

Unchecked, Harbor sends its external URL: Clair pulls the blobs through the public ingress and
follows the registry's 307 redirects to object storage, which is how it runs against R2 today.

One adapter can serve several Harbor instances. Every scan request carries its own registry URL
and token, so nothing in the deployment is bound to one Harbor.
...
**Every scan fails with `[network] ... no such host`.** Harbor is sending its internal core
hostname as the registry URL, and neither the adapter nor Clair can resolve it from outside
Harbor's namespace. Re-register the scanner with **Use internal registry addr** unchecked, as
described under [Registering with Harbor](#registering-with-harbor).

**An image scans clean and you expected findings.** Coverage is Clair's vulnerability data, not
something the adapter decides. Three cases account for nearly all of it:

* **End-of-life Debian.** Clair's Debian updater only carries the releases the live security
  tracker still lists, so jessie through bullseye match nothing. Bullseye left the tracker on
  2026-09-01 and its images went quiet the same day.
* **No detected distribution.** busybox, scratch-based and other distroless images have no
  package database to match against, so an empty report is the correct answer.
* **UBI and Oracle Linux.** Those need `rhel` and `oracle` in Clair's `updaters.sets`. Without
  them the layers index fine and nothing ever matches.

**How long a Scan All takes.** Measured on Harbor 2.16 over 125 artifacts, two adapter workers
against one Clair: 5 minutes from cold, 1:47 with Clair's index warm. Clair's index call was 4.6s
at p90 with a 193s worst case on `wordpress:6.4`. The adapter stayed under 130 MiB RSS throughout;
the memory worth sizing is Clair's.
