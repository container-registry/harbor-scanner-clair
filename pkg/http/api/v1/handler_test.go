...
func TestRequestHandler_AcceptScanRequest(t *testing.T) {
  ...
  "artifact": {"repository": "library/mongo", "digest": %q}
	...
	testCases := []struct {
		...
		{
			// "/v2/..." is appended to registry.url, so a query in it would be
			// sent in front of the path and the fetch would fail in the worker.
			name:                "Should respond with error 422 when registry URL carries a query",
			requestBody:         `{"registry":{"url":"https://core.harbor.domain?token=x"}}`,
			expectedStatus:      http.StatusUnprocessableEntity,
			expectedContentType: api.MimeTypeError.String(),
			expectedResponse:    errorJSON("invalid registry.url: expected an absolute http(s) URL"),
		},
		{
			name:                "Should respond with error 422 when registry URL carries an empty forced query",
		...
		{
			// Whitespace is not an anonymous pull: the raw value is forwarded as
			// the Authorization header of every layer fetch.
			name:                "Should respond with error 422 when the authorization is whitespace only",
			requestBody:         fmt.Sprintf(`{"registry":{"url":"https://c","authorization":"   "},"artifact":{"repository":"a","digest":%q}}`, validDigest),
			expectedStatus:      http.StatusUnprocessableEntity,
			expectedContentType: api.MimeTypeError.String(),
			expectedResponse: errorJSON("invalid registry.authorization; this adapter advertises Bearer registry authorization -- " +
				"a non-Bearer authorization indicates a misconfigured scanner registration"),
...
func TestRequestHandler_GetScanReportGzip(t *testing.T) {
	...
	assert.Equal(t, api.HeaderAcceptEncoding, rr.Header().Get(api.HeaderVary))
...
func TestRequestHandler_GetMetadata(t *testing.T) {
  ...
  "properties": {
	...
	// /metadata answers without the API key unless one is configured, so a
	// password in the Clair URL would be published to anyone who can reach it.
	t.Run("Should redact a password carried in the Clair URL", func(t *testing.T) {
		config := testConfig(t)
		config.Clair.URL = "https://scanner:s3cret@clair.example:6060"
		...
		var metadata harbor.ScannerAdapterMetadata
		require.NoError(t, json.Unmarshal(rr.Body.Bytes(), &metadata))
		assert.Equal(t, "https://scanner:xxxxx@clair.example:6060", metadata.Properties["env.SCANNER_CLAIR_URL"])
		assert.NotContains(t, rr.Body.String(), "s3cret")
	})
