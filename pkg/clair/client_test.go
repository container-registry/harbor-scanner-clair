...
// fixture is called from httptest handlers as well as from test bodies, so it
// asserts rather than requires: FailNow from a handler goroutine leaves the
// request hanging instead of reporting the missing file.
...
func TestClientVulnerabilityReport(t *testing.T) {
	...
	// Measured against a live Clair: the matcher 404s the report of a manifest
	// it has just indexed, and has it a moment later. At the full initial
	// interval that one 404 is two seconds of every cold scan.
	t.Run("retries the first 404 without paying the initial interval", func(t *testing.T) {
		t.Parallel()
		var calls atomic.Int32
		c := newTestClient(t, func(w http.ResponseWriter, _ *http.Request) {
			if calls.Add(1) == 1 {
		...
		// The production ladder, not the shrunk one: the point of the test is
		// that the first 404 does not climb it.
		c.retryInitial, c.retryMax = retryInitialInterval, retryMaxInterval
		c.reportRetryTimeout = 30 * time.Second
		...
		assert.Len(t, report.Vulnerabilities, 1)
		assert.Equal(t, int32(2), calls.Load())
		assert.Less(t, elapsed, time.Second, "the first 404 waited the full backoff")
	})
...
func TestClientVulnDBUpdatedAt(t *testing.T) {
	...
	// Without this the endpoint is probed again on every Harbor page load, so a
	// Clair that does not expose it at all is asked about it forever.
	t.Run("caches a refresh that failed with nothing cached", func(t *testing.T) {
		t.Parallel()
		var calls atomic.Int32
		c := newTestClient(t, func(w http.ResponseWriter, _ *http.Request) {
			calls.Add(1)
