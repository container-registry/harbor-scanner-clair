...
const (
	...
	// reportNotFoundFirstRetry is what the first 404 waits instead of
	// retryInitialInterval. Against a live Clair 4.9.0, 80 of 125 reports 404ed
	// on the first GET after a successful index and were there on the next one:
	// the matcher does not see the fresh index report for a moment. At the full
	// initial interval that one artifact cost two seconds of every cold scan.
	reportNotFoundFirstRetry = 250 * time.Millisecond
)
...
func (c *client) reportAttempt(ctx context.Context, manifestHash string, notFound *int) (*VulnerabilityReport, error) {
	...
	case http.StatusNotFound:
		*notFound++
		...
		after := c.retryAfter(resp)
		if after == 0 && *notFound == 1 {
			after = reportNotFoundFirstRetry
		}
		return nil, &retryableError{
...
func (c *client) VulnDBUpdatedAt(ctx context.Context) (time.Time, bool, error) {
	...
	c.mu.Lock()
	defer c.mu.Unlock()
	// The TTL is stamped on a failed refresh too. update_operation is
	// documented as internal and "may not exist" in combo mode, and Harbor
	// calls /api/v1/metadata on every scanner page load, so an endpoint that is
	// permanently absent would otherwise turn every page load into a request to
	// Clair.
	if err != nil {
		// A failure here is not an adapter fault: the metadata property is
		// dropped and the last good value keeps being served. A canceled or
		// timed-out caller says nothing about Clair, so it does not start the
		// negative-cache window; an answer from Clair (404, 403, decode) does.
		if !errors.Is(err, context.Canceled) && !errors.Is(err, context.DeadlineExceeded) {
			c.vulnDBFetchedAt = time.Now()
		}
		slog.Debug("Could not read Clair's latest vulnerability update operation",
			slog.String("err", err.Error()))
		return c.vulnDBAt, c.vulnDBOK, nil
	}
	c.vulnDBFetchedAt = time.Now()
	c.vulnDBAt, c.vulnDBOK = latest, true
