...
func (c *Client) GetManifest(ctx context.Context, req harbor.ScanRequest) (*Manifest, error) {
	...
	// One byte past the limit: reading exactly the limit cannot tell a manifest
	// that fits from one that was truncated into something still decodable.
	...
	if len(body) > maxManifestBytes {
		return nil, fmt.Errorf("%w: the manifest at %s is larger than %d bytes",
			ErrManifestFetch, url, maxManifestBytes)
	}
	if err := verifyDigest(req.Artifact.Digest, body); err != nil {
		return nil, fmt.Errorf("%w: %s: %w", ErrManifestFetch, url, err)
	}
...
// verifyDigest checks the body against the digest it was fetched by. The
// manifest is the trust anchor for the whole scan: Clair verifies each layer
// against the digests this body carries, so a body nobody verified means layers
// nobody verified, reported to Harbor under the digest it asked about.
func verifyDigest(want string, body []byte) error {
	parsed, err := digest.Parse(want)
	if err != nil {
		return fmt.Errorf("the requested digest %q is not a digest: %w", want, err)
	}
	if got := parsed.Algorithm().FromBytes(body); got != parsed {
		return fmt.Errorf("the registry returned a manifest with digest %s, not %s", got, parsed)
	}
	return nil
}
