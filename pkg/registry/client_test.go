...
type fixture struct {
	...
	client *Client
	// digest of the body served, which is how the client addresses it.
	digest string
...
func newFixture(t *testing.T, status int, contentType string, body []byte) *fixture {
	...
	f.digest = digest.FromBytes(body).String()
	return f
}
...
// withoutMediaType strips the body's own mediaType, which is what leaves the
// response header as the only thing identifying the manifest.
func withoutMediaType(t *testing.T, body []byte) []byte {
	t.Helper()
...
func scanRequest(registryURL, artifactDigest string) harbor.ScanRequest {
	return harbor.ScanRequest{
		Registry: harbor.Registry{URL: registryURL, Authorization: testAuth},
		Artifact: harbor.Artifact{Repository: testRepository, Digest: artifactDigest},
	}
}
...
func TestClient_GetManifest(t *testing.T) {
	t.Run("Should request the manifest by digest and forward the scan authorization", func(t *testing.T) {
		...
		manifest, err := f.client.GetManifest(context.Background(), scanRequest(f.server.URL, f.digest))
		require.NoError(t, err)

		assert.Equal(t, "/v2/"+testRepository+"/manifests/"+f.digest, f.requestPath)
		assert.Equal(t,
	...
	t.Run("Should not double the slash when the registry URL ends in one", func(t *testing.T) {
		...
		req := scanRequest(f.server.URL+"/", f.digest)
	...
	t.Run("Should accept an OCI manifest whose media type comes from the response header", func(t *testing.T) {
		f := newFixture(t, http.StatusOK, mediaTypeOCIManifest+"; charset=utf-8",
			withoutMediaType(t, testdata(t, "manifest_oci.json")))
	...
	// The manifest is fetched by digest and every layer digest the scan trusts
	// comes out of this body, so a body that is not the artifact Harbor asked
	// about is rejected before anything is indexed under that digest.
	t.Run("Should reject a manifest whose body does not match the requested digest", func(t *testing.T) {
		f := newFixture(t, http.StatusOK, mediaTypeDockerManifestV2, testdata(t, "manifest_schema2.json"))

		_, err := f.client.GetManifest(context.Background(), scanRequest(f.server.URL, testDigest))
		require.Error(t, err)
		assert.ErrorIs(t, err, ErrManifestFetch)
		assert.Contains(t, err.Error(), "not "+testDigest)
	})

	// The complete object sits inside the limit and only padding is cut, which
	// is the one shape a truncated read decodes as a valid manifest.
	t.Run("Should reject a manifest larger than the limit", func(t *testing.T) {
		body := append(testdata(t, "manifest_schema2.json"), bytes.Repeat([]byte(" "), maxManifestBytes)...)
		f := newFixture(t, http.StatusOK, mediaTypeDockerManifestV2, body)

		_, err := f.client.GetManifest(context.Background(), scanRequest(f.server.URL, f.digest))
		require.Error(t, err)
		assert.ErrorIs(t, err, ErrManifestFetch)
...
func TestManifest_ClairManifest(t *testing.T) {
	...
	req := scanRequest("https://harbor.example.com/", testDigest)
	manifest, err := f.client.GetManifest(context.Background(), scanRequest(f.server.URL, f.digest))
	require.NoError(t, err)
