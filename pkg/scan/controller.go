...
func (c *controller) Scan(ctx context.Context, jobID string, req harbor.ScanRequest) error {
	...
	if err != nil {
		...
		if uErr := c.store.UpdateStatus(writeCtx, jobID, job.Failed, errMsg); uErr != nil {
			// Same reasoning as the branch above: the record outlived its TTL
			// or reached a terminal state already, and neither is a failure of
			// this scan. Returning it would have the worker log the scan as
			// failed for a reason that has nothing to do with the scan.
			if errors.Is(uErr, persistence.ErrJobNotFound) {
				slog.Warn("Scan job record is gone; the Failed status could not be recorded",
					slog.String("scan_job_id", jobID), slog.String("err", errMsg))
				return nil
			}
