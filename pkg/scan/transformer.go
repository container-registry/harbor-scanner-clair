...
func toDistributionName(dist *clair.Distribution) string {
	...
	// Clair's distribution scanners fill version_id and leave version empty as
	// often as the other way round, so take whichever carries the release.
	version := dist.Version
	if version == "" {
		version = dist.VersionID
	}
	return strings.TrimSpace(dist.Name + " " + version)
}
