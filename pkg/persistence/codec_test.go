...
// TestDecodeRejectsOversizedPlaintext pins that the size cap holds for a row
// that did not come from EncodeReport. The report endpoint returns whatever
// Get hands back on every poll, so an uncompressed row has to be bounded too.
func TestDecodeRejectsOversizedPlaintext(t *testing.T) {
	oversized := bytes.Repeat([]byte("a"), maxReportBytes+1)
	_, err := DecodeReport(oversized)
	require.Error(t, err)
