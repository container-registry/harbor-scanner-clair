...
// TestTerminalWriteRenewsTheRetentionWindow pins what the TTL bounds after the
// scan is over: how long Harbor may poll the outcome, not how long the job had
// to run. A scan that spent most of its TTL queued and then failed must still be
// readable, or Harbor would poll a 404 instead of seeing the failure.
func TestTerminalWriteRenewsTheRetentionWindow(t *testing.T) {
	ctx := context.Background()

	// nearExpiry leaves a record one second short of aging out, which is the
	// state a job reaches after a long queue wait plus a long scan.
	nearExpiry := func(t *testing.T, pool *pgxpool.Pool, id string) {
		t.Helper()
		_, err := pool.Exec(ctx,
			`UPDATE scan_job SET expires_at = now() + interval '1 second' WHERE id = $1`, id)
		require.NoError(t, err)
	}
	remaining := func(t *testing.T, pool *pgxpool.Pool, id string) time.Duration {
		t.Helper()
		var secs float64
		require.NoError(t, pool.QueryRow(ctx,
			`SELECT extract(epoch from (expires_at - now())) FROM scan_job WHERE id = $1`, id).Scan(&secs))
		return time.Duration(secs * float64(time.Second))
	}

	t.Run("failed", func(t *testing.T) {
		s, pool := newTestStore(t)
		require.NoError(t, s.Create(ctx, queuedJob("r1")))
		require.NoError(t, s.UpdateStatus(ctx, "r1", job.Pending))
		nearExpiry(t, pool, "r1")

		require.NoError(t, s.UpdateStatus(ctx, "r1", job.Failed, "clair is down"))
		assert.Greater(t, remaining(t, pool, "r1"), 50*time.Minute,
			"a recorded failure must stay pollable for a full TTL")
	...
	t.Run("pending does not renew", func(t *testing.T) {
		s, pool := newTestStore(t)
		require.NoError(t, s.Create(ctx, queuedJob("r2")))
		nearExpiry(t, pool, "r2")

		require.NoError(t, s.UpdateStatus(ctx, "r2", job.Pending))
		assert.Less(t, remaining(t, pool, "r2"), time.Minute,
			"a reclaimed job must not buy itself another TTL")
	})
}
...
func TestDepthCountsOnlyWaitingJobs(t *testing.T) {
	...
	// The worker holding that claim is killed. Its row is Pending with a lapsed
	// lock, which claimStmt treats as runnable, so the gauge has to see it too
	// or a crashed replica's backlog reads as an empty queue.
	_, err = pool.Exec(ctx,
		`UPDATE scan_job SET locked_until = now() - interval '1 second' WHERE id = $1`, claim.ID)
	require.NoError(t, err)
	depth, err = s.Depth(ctx)
	require.NoError(t, err)
	assert.Equal(t, int64(3), depth, "a job whose worker died is waiting again")

	expire(t, pool, claim.ID)
	depth, err = s.Depth(ctx)
	require.NoError(t, err)
	assert.Equal(t, int64(2), depth, "an expired job is gone, not waiting")
}
