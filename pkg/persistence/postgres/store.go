...
ON CONFLICT (id) DO NOTHING`
	...
	// A terminal write also extends expires_at, for the same reason finishStmt
	// does: the TTL then bounds how long Harbor may poll the outcome rather than
	// how long the job had to run. Without it a scan that failed late in its TTL
	// would 404 on the next poll instead of reporting the failure. Pending is
	// deliberately excluded: renewing on every attempt would let a job that
	// keeps being reclaimed outlive the TTL that is meant to bound it.
	updateStatusStmt = `
UPDATE scan_job
   SET status = $2::smallint,
       error = COALESCE($3, error),
       locked_until = CASE WHEN $2::smallint IN (2, 3) THEN NULL ELSE locked_until END,
       expires_at = CASE WHEN $2::smallint IN (2, 3)
...
RETURNING id, request, attempts, created_at`
	...
	// depthStmt mirrors the runnable predicate of claimStmt rather than counting
	// Queued alone: a row whose worker was killed is Pending with a lapsed lock,
	// and it is waiting for a worker just as much as a queued one. Counting only
	// status 0 would report an empty queue while a crashed replica's jobs sit
	// there waiting to be reclaimed. Expired rows stay out: they are invisible
	// to claimStmt and Sweep deletes them, so counting them would report a
	// backlog that no worker can drain.
	depthStmt = `
SELECT count(*) FROM scan_job
 WHERE (status = 0 OR (status = 1 AND locked_until < now()))
   AND expires_at > now()`
)
...
func (s *Store) UpdateStatus(ctx context.Context, scanJobID string, newStatus job.ScanJobStatus, errorMsg ...string) error {
	...
	tag, err := s.pool.Exec(ctx, updateStatusStmt, scanJobID, int16(newStatus), msg, s.cfg.ScanJobTTL.Seconds())
	if err != nil {
...
// Depth counts the jobs a worker could claim right now: queued ones plus those
// left Pending by a worker that died. It backs the queue_depth gauge and runs on
// the Prometheus scrape goroutine, so it respects ctx.
