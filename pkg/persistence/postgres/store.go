...
RETURNING id, request, attempts, created_at`
	...
	// Expired rows are invisible to claimStmt and Sweep deletes them, so
	// counting them would report a backlog that no worker can drain.
	depthStmt = `SELECT count(*) FROM scan_job WHERE status = 0 AND expires_at > now()`
)
