...
func DecodeReport(stored []byte) (json.RawMessage, error) {
	...
	if !bytes.HasPrefix(stored, gzipMagic) {
		// Everything this adapter writes is gzipped, so an uncompressed row was
		// written by hand or by an older version. The report endpoint returns
		// this on every poll, so the cap applies here too.
		if len(stored) > maxReportBytes {
			return nil, fmt.Errorf("stored scan report is %d bytes, over the %d limit", len(stored), maxReportBytes)
		}
		return stored, nil
	}
