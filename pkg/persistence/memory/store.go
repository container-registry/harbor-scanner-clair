...
func (s *store) Create(_ context.Context, scanJob job.ScanJob) error {
	...
	// Copy the report for the same aliasing reason Get and Finish copy it.
	if scanJob.Report != nil {
		scanJob.Report = append(json.RawMessage(nil), scanJob.Report...)
	}
...
func (s *store) UpdateStatus(_ context.Context, scanJobID string, newStatus job.ScanJobStatus, errorMsg ...string) error {
	...
	// updateStatusStmt carries `status IN (0, 1)`: a terminal record is final,
	// and a late write from a replayed job must not reopen it. Refusing it the
	// same way keeps a test against the memory backend honest.
	if j.Status == job.Finished || j.Status == job.Failed {
		return fmt.Errorf("scan job (%s) is already %s: %w", scanJobID, j.Status, persistence.ErrJobNotFound)
	}
	j.Status = newStatus
	if len(errorMsg) > 0 {
		j.Error = errorMsg[0]
...
func (s *store) Finish(_ context.Context, scanJobID string, report json.RawMessage) error {
	...
	// finishStmt carries `status = 1`: only the worker that holds the job can
	// finish it, and only once.
	if j.Status != job.Pending {
		return fmt.Errorf("scan job (%s) is %s, not pending: %w", scanJobID, j.Status, persistence.ErrJobNotFound)
	}
