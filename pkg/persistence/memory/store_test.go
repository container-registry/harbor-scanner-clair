...
func TestMemoryStoreFailIfQueued(t *testing.T) {
	...
	got, err = s.Get(ctx, jobID)
	require.NoError(t, err)
	assert.Equal(t, job.Failed, got.Status, "a non-Queued record must be left untouched")
}
...
// TestMemoryStoreTerminalStatesAreFinal mirrors the WHERE clauses in
// updateStatusStmt and finishStmt: a record that reached Finished or Failed is
// final, so a replayed job cannot reopen it or overwrite its report.
func TestMemoryStoreTerminalStatesAreFinal(t *testing.T) {
	ctx := context.Background()
	s := NewStore()
	require.NoError(t, s.Create(ctx, job.ScanJob{ID: jobID, Status: job.Queued}))
	require.NoError(t, s.UpdateStatus(ctx, jobID, job.Pending))
	...
	require.ErrorIs(t, s.UpdateStatus(ctx, jobID, job.Pending), persistence.ErrJobNotFound)
	require.ErrorIs(t, s.Finish(ctx, jobID, json.RawMessage(`{"b":2}`)), persistence.ErrJobNotFound)

	got, err := s.Get(ctx, jobID)
	require.NoError(t, err)
	assert.Equal(t, job.Finished, got.Status)
	assert.JSONEq(t, `{"a":1}`, string(got.Report))
}

// TestMemoryStoreFinishRequiresPending mirrors `status = 1` in finishStmt: only
// the worker that claimed the job may finish it.
func TestMemoryStoreFinishRequiresPending(t *testing.T) {
	ctx := context.Background()
	s := NewStore()
	require.NoError(t, s.Create(ctx, job.ScanJob{ID: jobID, Status: job.Queued}))
	require.ErrorIs(t, s.Finish(ctx, jobID, json.RawMessage(`{"a":1}`)), persistence.ErrJobNotFound)
}
