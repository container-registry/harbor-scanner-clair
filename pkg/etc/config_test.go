...
// The manifest GET is bounded by the request timeout and runs inside the job
// deadline, so raising it past the fixed overhead has to raise the budget.
func TestJobBudgetFollowsTheRequestTimeout(t *testing.T) {
	setenvs(t, Envs{
		"SCANNER_STORE_POSTGRES_URL":    testDSN,
		"SCANNER_CLAIR_REQUEST_TIMEOUT": "90s",
	})
	cfg, err := GetConfig()
	require.NoError(t, err)

	assert.Equal(t, 16*time.Minute+30*time.Second, cfg.JobDeadline())
}
