...
const (
	// jobOverhead is the floor for the manifest GET and the status write that
	// bracket the two Clair budgets. A larger request timeout replaces it.
...
// call, the report retry loop, and the overhead around both. The manifest GET
// runs inside the same deadline and is bounded by the request timeout, so a
// request timeout above the fixed overhead has to widen the budget with it.
func (c Config) JobDeadline() time.Duration {
	return c.Clair.IndexTimeout + c.Clair.ReportRetryTimeout + max(c.Clair.RequestTimeout, jobOverhead)
}
...
func validateClairURL(value string) error {
	...
	// Hostname(), not Host: "https://:6060" parses to a non-empty Host that has
	// nothing to dial.
