...
## What the updater sets cover

`updaters.sets` in `clair.yaml` is the whole of Clair's coverage, and a set you
did not configure is not an error anywhere. An image from an uncovered
distribution indexes normally and comes back with an empty report, so zero
findings means "no data for this distribution" as readily as it means "nothing
to find".

The same trap lives inside a configured set: an image built on an end-of-life
Debian scans clean, because the security tracker the `debian` updater follows
carries the suites Debian still supports.

A set name Clair does not know is accepted in silence: `clairctl check-config`
passes it, and Clair starts and logs `running updaters` with `total: 0`. A typo
costs a whole distribution with nothing in the logs to say so.

Two sets are worth knowing before you add them. Measured against 4.9.0 on
2026-09-15, one container, empty database:

| Set | Fetches | First cycle |
|-----|---------|-------------|
| `oracle` | 20 per-year OVAL files (`com.oracle.elsa-<year>.xml.bz2`), 12 MiB compressed | 14 s, 814 MiB peak RSS |
| `rhel-vex` | Red Hat's CSAF VEX archive, 302 MiB compressed, 54k advisories | still fetching after 4.5 min, 693 MiB peak RSS |

`rhel` is **not** a set name in 4.9.0: the RHEL updater registers as
`rhel-vex`, and `rhel` is one of those silently empty sets. `oracle` fits inside
the 2Gi limit in `clair.yaml` with room to spare; `rhel-vex` was still climbing
when that measurement stopped, so watch the pod through its first cycle before
you trust the limit.
...
## Register it in Harbor

Administration -> Interrogation Services -> Scanners -> NEW SCANNER, with
`http://harbor-scanner-clair.harbor.svc:8080` as the endpoint.

Leave **Use internal registry addr** off (`use_internal_addr: false` when you
register over the API). With it on, Harbor replaces the registry URL in every
scan request with its own internal core address, and that name has to resolve
wherever the blobs are pulled - which here is the `clair` namespace, not
Harbor's. Scans then fail with a `no such host` network error naming Harbor's
core service.

One adapter release can serve several Harbor instances. It keeps no per-Harbor
state: each scan request carries its own registry URL and bearer token, so
registering the same endpoint in every Harbor is enough.
