...
5. **The chart tracks the adapter, not Clair.** Every value must correspond to
   something `pkg/etc/config.go` actually reads. A knob for a Clair server
   setting belongs in whatever deploys Clair, which is not this chart.
...
or removing a value means a `feat!:` commit and a note in the release, because
the closed schema turns a rename into a hard render failure for every existing
user. That is the intended behaviour, but it only helps if the changelog says
what to do instead.
