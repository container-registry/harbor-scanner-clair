...
Leave **Use internal registry addr** off unless Harbor, the adapter and Clair
all sit in one namespace (`use_internal_addr: false` when you register over the
API). It makes Harbor put its own internal core address in the registry URL of
every scan request, and that name has to resolve for Clair as well, since Clair
is what pulls the layer blobs. One release can serve several Harbor instances:
the adapter holds no per-Harbor state, so registering the same endpoint in each
is all it takes.
...
- **Jobs are durable.** The queue is the `scan_job` table: a worker claims a
  ...
  expires. SIGTERM is that same path, deliberately: the worker stops claiming,
  cancels the scans it holds and waits up to 10 seconds for them to unwind, and
  each cancelled row is re-run by whichever worker claims it after the lock
  expires. `terminationGracePeriodSeconds` above 10 seconds only bounds the
  kubelet's wait.
...
**Every scan fails with a network error naming Harbor's core service.** The
scanner is registered with **Use internal registry addr** on, so Harbor sends
its internal core address as the registry URL. That name resolves inside
Harbor's namespace and nowhere else, and it has to resolve for Clair too.
Re-register the scanner with `use_internal_addr: false`.
...
| terminationGracePeriodSeconds | int | `60` | Grace period for a terminating pod. On SIGTERM the worker stops taking new jobs and cancels the in-flight ones, then waits up to 10 seconds for them to unwind; a cancelled scan leaves its row for the next worker to claim rather than finishing. Anything above 10 seconds only bounds how long the kubelet waits before SIGKILL. |
