...
def check(values):
    ...
    # a queued job. A TTL under the job deadline turns a slow-but-successful
    # scan into a 404.
    #
    # The deadline is the whole job, not just the index call: pkg/etc/config.go
    # builds it as IndexTimeout + ReportRetryTimeout + 30s of overhead, and the
    # adapter logs the same warning at startup. Queue wait comes on top, which
    # is why the chart default is an hour rather than the bare minimum.
    ttl = parse_duration(
    ...
    report_retry_timeout = parse_duration(
        effective(values, "clair.reportRetryTimeout", "SCANNER_CLAIR_REPORT_RETRY_TIMEOUT", "5m")
    )
    if None not in (ttl, index_timeout, report_retry_timeout):
        job_deadline = index_timeout + report_retry_timeout + 30.0
        if ttl <= job_deadline:
            warn(
                "store.scanJobTTL",
                "is not longer than the adapter's job deadline "
                "(clair.indexTimeout + SCANNER_CLAIR_REPORT_RETRY_TIMEOUT + "
                "30s), so a scan that runs to that deadline expires before "
                "Harbor can collect the report.",
            )
