...
2. Build the image and start it on an address the Harbor core container can reach. `task info`
   ...
   docker run --rm -p 8080:8080 \
     --network harbor-scanner-clair-component_default \
   ...
   ```
   All URLs are resolved from inside the container, which is why it joins the compose network:
   `clair` and `postgres` are service names on it, and a container on the default bridge cannot
   resolve them. That DSN is the one `task compose:up` provisions: the tier's
   `postgres/init.sql` creates a `scanner` database next to Clair's own.
