...
func waitForAdapter() error {
	...
	for {
		...
		if time.Now().After(deadline) {
			// lastStatus as well as err: a container that answers 503 for two
			// minutes leaves err nil, and the status is the whole diagnosis.
			return fmt.Errorf("no 200 from %s/probe/healthy within 2m (last status %d, last error: %v)",
				adapterURL, lastStatus, err)
		}
